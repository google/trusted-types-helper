...
export async function openDevToolsPanel(
  ...
  const extensionPanelView = await devTools.evaluate(
    `Object.keys(UI.panels).find(key => key.startsWith('chrome-extension://'))`,
  );
  await devTools.evaluate(
    `UI && UI.viewManager ? UI.viewManager.showView('${extensionPanelView}') : InspectorFrontendAPI.showPanel('${extensionPanelView}')`,
  );
